...
def train_transformer_model(model_instance, X_train, X_val, X_test, y_train, y_val, y_test, num_epochs, dataset_name):
    classes = np.unique(y_train)
    class_weights = compute_class_weight('balanced', classes=classes, y=y_train)
    class_weight = {i: w for i, w in enumerate(class_weights)}

    model_path = f'/your_path/EnDeep4mC/pretrained_models/indiv_14_feature/transformer_{dataset_name}.h5'
    callbacks = [
    ...
    y_pred = (model_instance.model.predict(X_test) > 0.5).astype(int).flatten()
    y_proba = model_instance.model.predict(X_test).flatten()
...
def save_roc_data(dataset, model_name, fpr, tpr, auc):
    ...
    roc_file = os.path.join(project_dir, 'pretrained_models/indiv_14_roc', f'{model_name}_roc_{dataset}.csv')
...
def plot_roc_curve(dataset, model_name, fpr, tpr, auc):
    ...
    plt.savefig(os.path.join(project_dir, 'pretrained_models/indiv_14_roc', f'{model_name}_roc_{dataset}.png'))
    plt.close()
...
if __name__ == "__main__":
    ...
    for dataset in fasta_dataset_names:
        ...
        y_pred, y_proba, history = train_transformer_model(
            transformer, X_train, X_val, X_test,
        ...
        dataset_info = {
            'Dataset': dataset,
            'Transformer_Test_ACC': f'{acc:.6f}',
            'Transformer_SN': f'{sn:.6f}',
            'Transformer_SP': f'{sp:.6f}',
            'Transformer_F1': f'{f1:.6f}',
            'Transformer_MCC': f'{mcc:.6f}',
            'Transformer_AUC': f'{auc:.6f}'
        }
    ...
    print("\nFinal test result:")
    print(summary_df)
    summary_df.to_csv('/your_path/EnDeep4mC/pretrained_models/indiv_14_feature/transformer_auto_summary.csv', index=False)